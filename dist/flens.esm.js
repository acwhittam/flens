...
var toFuture = unless(isFuture, resolve);
var getter = function(l) {
    return function(data) {
        return compose(chain(function(value) {
            return isNil(value) ? reject(data) : resolve(value);
        }), chain(function(value) {
            return toFuture(value);
        }), map(view(l)), toFuture)(data);
    };
};
var setter = function(l) {
    return function(value, target) {
        return compose(chain(function(param) {
            var _param = _sliced_to_array(param, 2), v = _param[0], t = _param[1];
            return toFuture(set(l, v, t));
        }), both(toFuture(value)))(toFuture(target));
    };
};
var wrap = function(l) {
    return lens(getter(l), setter(l));
};
var flens = compose(wrap, lens);
var flensProp = function(prop) {
    return wrap(lensProp(prop));
};
var flensPath = function(path) {
    return wrap(lensPath(path));
};
var flensIndex = function(index) {
    return wrap(lensIndex(index));
};
